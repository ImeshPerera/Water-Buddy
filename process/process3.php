...
if(isset($_POST["extariff"]) && !empty($_POST["extariff"])){
    $query ="SELECT * FROM bill_tariff RIGHT JOIN water_bill_units ON bill_tariff.id = water_bill_units.tariff_id WHERE bill_tariff.id = '".$_POST["extariff"]."';";
    $resultset = $dbms -> query($query);
    $n = $resultset->num_rows;
    $continueon = true;
}elseif(isset($_POST["year"]) && isset($_POST["month"])){
    $dateStr = $_POST["year"]." ".$_POST["month"]." 01";
    $date = DateTime::createFromFormat('Y F d', $dateStr);
    $formattedDate = $date->format('Y-m-d');

    $query ="SELECT * FROM bill_tariff RIGHT JOIN water_bill_units ON bill_tariff.id = water_bill_units.tariff_id WHERE bill_tariff.upfrom <= '".$formattedDate."' AND (bill_tariff.tilfor >= '".$formattedDate."' OR bill_tariff.tilfor IS NULL);";
    $resultset = $dbms -> query($query);
    $n = $resultset->num_rows;
    $continueon = true;
}else{
